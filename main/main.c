...
void app_main(void)
{
  ...
  mq_params_data_t mq_params = {.channel = ADC_CHANNEL};

  dht_params_data_t dht_params = {.gpio = DHT22_GPIO};

  xTaskCreatePinnedToCore(mq_sensor_task, "mq_sensor_task", 4096, &mq_params, 5,
                          NULL, 0);
  ESP_LOGI(TAG, "Задача MQ-135 создана");

  xTaskCreatePinnedToCore(dht22_task, "dht22_task", 4096, &dht_params, 5, NULL,
                          1);
